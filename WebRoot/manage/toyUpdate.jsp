...
<% 
	...
	String type = "toy";	
	//保存的请求地址
	String saveAction = basePath + "toy/toy_save.action";
...
<div class="manage_right">
	<div class="content">
		...
		<form action="<%=saveAction %>" method="post" enctype="multipart/form-data" name="bookForm" id="bookForm">
				<input type="hidden" name="toy.id" value="<%=toy.getId() %>"/>
				<table>
					<tr>
						<td>名称：</td>
						<td><input type="text" name="toy.name" value="<%=toy.getName()%>"/></td>
					</tr>
					<tr>
						<td>颜色：</td>
						<td><input type="text" name="toy.color"  value="<%=toy.getColor()%>"/></td>
					</tr>
					<tr>
						<td>价格：</td>
						<td><input type="text" name="toy.price"  value="<%=toy.getPrice()%>"/></td>
					</tr>
					<tr>
						<td>图片：</td>
