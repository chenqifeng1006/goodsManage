...
<% 
	...
	String type = "book";	
	//保存的请求地址
	String saveAction = basePath + "book/book_save.action";
...
<div class="manage_right">
	<div class="content">
		...
		<form action="<%=saveAction %>" method="post" enctype="multipart/form-data" name="bookForm" id="bookForm">
				...
				<input type="hidden" name="book.creationtime" value="<%=book.getCreationtime() %>"/>
				<table>
					<tr>
						<td>名称：</td>
						<td><input type="text" name="book.name" value="<%=book.getName()%>"/></td>
					</tr>
					<tr>
						<td>作者：</td>
						<td><input type="text" name="book.author"  value="<%=book.getAuthor()%>"/></td>
					</tr>
					<tr>
						<td>出版社：</td>
					...
					<tr>
						<td>价格：</td>
						<td><input type="text" name="book.price"  value="<%=book.getPrice()%>"/></td>
					</tr>
					<tr>
						<td>图片：</td>
