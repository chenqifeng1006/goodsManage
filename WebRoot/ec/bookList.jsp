...
<div class="w">
	<div class="w100">
	    ...
	    <div class="ec_content">
	    	...
	    	<div class="block">
				<img src="<%=basePath%><%=bookList.get(i).getFileid()%>" width="140" height="200">
				<div class="info">
					
					<div class="title">名       称：<%=bookList.get(i).getName() %></div>
					<div class="author">作     者：<%=bookList.get(i).getAuthor() %></div>
					<div class="time">出版时间：<%=bookList.get(i).getCreationtime() %></div>
					<div class="publisher">出版社：<%=bookList.get(i).getPublisher() %></div>				
					<div class="price">价格：￥<%=bookList.get(i).getPrice() %></div>
					<a href="<%=basePath%>shoppingCar/shoppingCar_add.action?commodityid=<%=bookList.get(i).getId() %>&type=book&price=<%=bookList.get(i).getPrice()%>">
