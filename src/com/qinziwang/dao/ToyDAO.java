...
public class ToyDAO {
    ...
    /*添加信息*/
    public void add(Toy toy) throws Exception {
        Session s = null;
        Transaction tx = null;
        try { 
    ...
    /*查询信息*/
    public ArrayList<Toy> Query(int currentPage,int pageSize) { 
        Session s = null; 
        try {
            s = HibernateUtil.getSession();
            String hql = "From Toy toy where 1=1";
/*            if(null != member && !member.getUserNo().equals("")) hql += " and inputCashTable.member.userNo='" + member.getUserNo() + "'";
    ...
    public Toy GetById(int id) {
        ...
        try {
            s = HibernateUtil.getSession();
            Toy toy = (Toy)s.get(Toy.class, id);
            return toy;
        } finally {
            HibernateUtil.closeSession();
        }
