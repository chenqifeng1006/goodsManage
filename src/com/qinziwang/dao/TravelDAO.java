...
public class TravelDAO {
    ...
    /*查询信息*/
    public ArrayList<Travel> Query(int currentPage,int pageSize) { 
        Session s = null; 
        try {
            s = HibernateUtil.getSession();
            String hql = "From Travel travel where 1=1";
/*            if(null != member && !member.getUserNo().equals("")) hql += " and inputCashTable.member.userNo='" + member.getUserNo() + "'";
    ...
    public Travel GetById(int id) {
        ...
        try {
            s = HibernateUtil.getSession();
            Travel travel = (Travel)s.get(Travel.class, id);
            return travel;
        } finally {
            HibernateUtil.closeSession();
        }
    ...
    public void Delete (int id) throws Exception {
        ...
        try {
            s = HibernateUtil.getSession();
            tx = s.beginTransaction();
            Object obj = s.get(Travel.class, id);
            s.delete(obj);
            tx.commit();
        } catch (HibernateException e) {
            if(tx != null)
