...
public class BorrowGoodsDAO {
    ...
    /*添加图书信息*/
    public void add(BorrowGoods entity) throws Exception {
        Session s = null;
        Transaction tx = null;
        try { 
    ...
    /*查询信息*/
    public ArrayList<BorrowGoods> query(int currentPage,int pageSize,String goodsname) { 
        Session s = null; 
        try {
            s = HibernateUtil.getSession();
            String hql = "From BorrowGoods borrowGoods where 1=1";
...
/*            if(null != member && !member.getUserNo().equals("")) hql += " and inputCashTable.member.userNo='" + member.getUserNo() + "'";
            ...
            List list = q.list();
            return (ArrayList<BorrowGoods>) list;
        } finally {
            HibernateUtil.closeSession();
        }
    }
    ...
    /*根据主键获取对象*/
    public BorrowGoods getById(int id) {
        Session s = null;
        try {
            s = HibernateUtil.getSession();
            BorrowGoods entity = (BorrowGoods)s.get(BorrowGoods.class, id);
            return entity;
        } finally {
            HibernateUtil.closeSession();
        }
    ...
    /*更新InputCashTable信息*/
    public void update(BorrowGoods entity) throws Exception {
        Session s = null;
        Transaction tx = null;
        try {
            s = HibernateUtil.getSession();
    ...
    public void delete (int inputId) throws Exception {
        ...
        try {
            s = HibernateUtil.getSession();
            tx = s.beginTransaction();
            Object obj = s.get(BorrowGoods.class, inputId);
            s.delete(obj);
            tx.commit();
        } catch (HibernateException e) {
            if(tx != null)
